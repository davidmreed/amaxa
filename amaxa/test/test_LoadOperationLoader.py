...
class test_LoadOperationLoader(unittest.TestCase):
    ...
    def test_LoadOperationLoader_validates_file_against_field_scope_missing_fields(self):
        ex = {
            'version': 1,
            'operation': [
                {
    ...
    def test_LoadOperationLoader_validates_file_against_field_group_with_strict(self):
        ...
        fieldnames = ['Id', 'Name']
        context_fieldnames = amaxa.LoadOperation(MockSimpleSalesforce()).get_filtered_field_map(
            'Account', lambda f: f['createable'] and f['type'] not in ['location', 'address', 'base64']).keys()
        self._run_error_validating_test(
            ex,
            [
                'Input file for sObject {} does not match specified field scope.\nScope: {}\nFile Columns: {}\n'.format(
    ...
    def test_LoadOperationLoader_returns_error_unsupported_fields(self):
        ...
        self._run_error_validating_test(
            ex,
            ['Field {}.{} is of an unsupported type (base64)'.format('Attachment', 'Body')]
        )
    ...
    @unittest.mock.patch('csv.DictWriter.writeheader')
    def test_LoadOperationLoader_does_not_truncate_or_write_headers_on_resume(self, dict_writer):
        ex = {
            'version': 1,
            'operation': [
                { 
