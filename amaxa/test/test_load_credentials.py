...
class test_CredentialLoader(unittest.TestCase):
    def _run_validation_test(self, input_data):
        with patch('simple_salesforce.Salesforce'):
            credential_loader = loader.CredentialLoader(
                input_data
            )
    ...
    def test_load_credentials_uses_username_password(self):
    ...
    @patch('requests.post')
    @patch('jwt.encode')
    def test_load_credentials_uses_jwt_key(self, jwt_mock, requests_mock):
    ...
    @patch('requests.post')
    @patch('jwt.encode')
    def test_load_credentials_uses_jwt_file(self, jwt_mock, requests_mock):
        requests_mock.return_value.status_code = 200
        requests_mock.return_value.json = Mock(
            return_value = {
                'instance_url': 'test.salesforce.com',
    ...
    @patch('requests.post')
    @patch('jwt.encode')
    def test_load_credentials_returns_error_on_jwt_key_failure(self, jwt_mock, requests_mock):
    ...
    @patch('requests.post')
    @patch('jwt.encode')
    def test_load_credentials_returns_error_on_jwt_file_failure(self, jwt_mock, requests_mock):
        body = { 'error': 'bad JWT', 'error_description': 'key error' }
        requests_mock.return_value.status_code = 400
        requests_mock.return_value.json = Mock(
            return_value = body
    ...
    def test_load_credentials_uses_access_token(self):
        self._run_authentication_test(
            {
                'version': 1,
                'credentials': {
    ...
    def test_load_credentials_returns_validation_errors(self):
        self._run_failure_test(
            {
                'credentials': {
    ...
    def test_load_credentials_returns_error_without_credentials(self):
        self._run_failure_test(
            {
                'version': 1,
                'credentials': {
