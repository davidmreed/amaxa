...
EXAMPLE_ID_MAP = {amaxa.SalesforceId(k): amaxa.SalesforceId(v) for k, v in EXAMPLE_DICT['state']['id-map'].items()}
...
class test_StateLoader(unittest.TestCase):
    ...
    def test_saves_state_json(self):
        ...
        self.assertEqual(
            EXAMPLE_DICT,
            yaml.safe_load(save_state(operation))
        )
    ...
    def test_saves_state_yaml(self):
        ...
        self.assertEqual(
            EXAMPLE_DICT,
            json.loads(save_state(operation, json_mode=True))
        )
